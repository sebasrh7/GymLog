...
export const EditarRutinaScreen = () => {
  ...
  const agregarEjercicio = (ej: Ejercicio) => {
    ...
    setEjerciciosConfig(prev => [
      ...prev,
      { ejercicio: ej, series: '3', repeticiones: '10', pesoObjetivo: '0', descanso: '90', tipoSerie: 'normal' },
    ]);
  ...
  const guardar = async () => {
    ...
    setGuardando(true);
    try {
      await rutinaService.actualizar(rutinaId, nombre.trim(), dia || undefined);
      const rutina = await rutinaService.getById(rutinaId);
      for (const ej of rutina?.ejercicios ?? []) {
        await rutinaService.eliminarEjercicioDeRutina(ej.id!);
      }
